...
const pageTranslations = {
  en: {
    ...
    formSubtitle: "We'd love to hear from you. Fill out the form below and we'll get back to you as soon as possible.",
    ...
    infoSubtitle: "Reach out through any of these channels and we'll respond within 24 hours.",
    phone: { title: "Phone", details: "+1 (555) 123-4567" },
    email: { title: "Email", details: "hello@photographystudio.com" },
    location: { title: "Location", details: "123 Creative Street, Art City, AC 12345" },
  },
  he: {
    pageTitle: "צור קשר",
    pageSubtitle: "בואו ניצור משהו מדהים ביחד",
    formTitle: "שלח לנו הודעה",
    formSubtitle: "נשמח לשמוע ממך. מלא את הטופס למטה ואנו נחזור אליך בהקדם האפשרי.",
    ...
    phone: { title: "טלפון", details: "+1 (555) 123-4567" }, // Details can be static
    email: { title: "אימייל", details: "hello@photographystudio.com" },
    location: { title: "מיקום", details: "רחוב היצירה 123, עיר האמנות, א\"ע 12345" },
  }
};
    ...
    <div className="min-h-screen" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <section className="relative h-[40vh] overflow-hidden">
        ...
        <div className="relative z-20 h-full flex items-center justify-center text-center">
         ...
         <motion.div initial={{ scale: 1.2 }} animate={{ scale: 1 }} transition={{ duration: 1.5 }} className="absolute inset-0">
          ...
          <img src="https://images.unsplash.com/photo-1486312338219-ce68d2c6f44d?ixlib=rb-4.0.3&auto=format&fit=crop&w=1000&q=80" alt="Contact" className="w-full h-full object-cover"/>
        </motion.div>
      </section>
      ...
      <section className="py-20">
        <div className="container mx-auto px-4">
          <div className={`grid md:grid-cols-2 gap-12 ${isRTL ? 'md:grid-flow-col-dense' : ''}`}>
            {/* Contact Form */}
            <motion.div
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className={`grid grid-cols-2 gap-4 ${isRTL ? 'grid-flow-col-dense' : ''}`}>
                  <div className={`space-y-2 ${isRTL ? 'col-start-2' : ''}`}>
                    ...
                    <Input required id="firstName" placeholder={t.firstNamePlaceholder} />
                  </div>
                  <div className={`space-y-2 ${isRTL ? 'col-start-1' : ''}`}>
                    <label htmlFor="lastName" className="text-sm font-medium">{t.lastNameLabel}</label>
                    <Input required id="lastName" placeholder={t.lastNamePlaceholder} />
                  </div>
                </div>

                <div className="space-y-2">
                  <label htmlFor="email" className="text-sm font-medium">{t.emailLabel}</label>
                  <Input required type="email" id="email" placeholder={t.emailPlaceholder} />
                </div>

                <div className="space-y-2">
                  <label htmlFor="message" className="text-sm font-medium">{t.messageLabel}</label>
                  <Textarea required id="message" placeholder={t.messagePlaceholder} className="h-32"/>
                </div>

                <Button type="submit" disabled={isSubmitting} className="w-full bg-primary hover:bg-primary/90">
                  {isSubmitting ? (
                    <motion.div initial={{ rotate: 0 }} animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity }}>
              ...
              <div className="space-y-6">
                {contactInfo.map((info, index) => (
                  <motion.div
                    ...
                    className={`flex items-start gap-4 p-4 rounded-lg bg-accent ${isRTL ? 'flex-row-reverse' : ''}`}
                  >
